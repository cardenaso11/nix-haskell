# An option whose value is a function rather than a setting. The manual
# carries both kinds in one list, and a type alone is easy to read past.
# The description therefore says which kind it is before anything else.
#
...
let # The trailing blank line makes what follows start its own paragraph
    # instead of running on from the label.
    marker = ''
      **A function, not a setting.** A project calls it and uses the
      result. Assign it only to replace what the call does.
